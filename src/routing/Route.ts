...
import type { RouteRegistrar } from "./RouteRegistrar.ts";
...
export class Route {
  ...
  public resource(name: string, controller: Class): void {
    this.prefix(name).name(`${name}.`).group(() => {
      ...
      for (const route of routes) {
        // @ts-ignore:
        new Route().name(route.name)[route.method](route.path, [
          controller,
          route.action,
        ]);
      }
  ...
  protected buildRouteStack(): AsyncResponseHandler {
    const handler: Exclude<RouteHandler, ControllerHandler> =
      Array.isArray(this.handler)
    ...
    let stack: AsyncResponseHandler = async (request: ChassisRequest) =>
      await handler(request);

    for (const mw of middleware) {
      const currentStack = stack;
      stack = async (request: ChassisRequest) =>
        await new mw().handle(request, currentStack);
    }
