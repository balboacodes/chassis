...
export class ChassisResponse extends Response {
   ...
   */
  public header(
    header: Header | Record<Header, string>,
    value?: string,
  ): ChassisResponse {
    ...
    header =
      (typeof header === "string" ? { [header]: value } : header) as Record<
        Header,
        string
      >;
  ...
  public async view(
    ...
    let file = await Deno.readTextFile(
      join(Deno.cwd(), "resources/views", `${view}.html`),
    );
