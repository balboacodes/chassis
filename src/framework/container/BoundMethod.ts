...
export default class BoundMethod {
    ...
    public static call(
        ...
        return BoundMethod.callBoundMethod(
            container,
            callback,
            () => callback(...array_values(BoundMethod.getMethodDependencies(container, callback, parameters))),
        );
    }
     ...
     */
    protected static callBoundMethod(
        container: Container,
        callback: (() => unknown) | [object, string],
        ...
        const method = BoundMethod.normalizeMethod(callback);
     ...
     */
    protected static normalizeMethod(callback: [object | string, string]): string {
        const className = typeof callback[0] === 'string' ? callback[0] : callback[0].constructor.name;
